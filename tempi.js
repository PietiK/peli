...
const {addPlayer, getPlayers, nextTurn, currentTurn, currentBuyer, nextBuyer, disconnectPlayer , setBuyPhase, getBuyPhase, getPlayersocks, getPlayermoney, getPlayer, getPlayerinv} = createPlayers();
/*
//Make a new player
...
io.on('connection', (sock) => {
    ...
    if(sock.id === currentBuyer()){ //Check if it's this player's turn to buy
      ...
      if(thiscard.tyyppi === 'ship') {  //If ship -> sell
        ...
        if(getPlayermoney(sock.id) >= thiscard.price) {
          ...
          io.to(sock.id).emit('addtoinv', cardId);
  ...
  sock.on('get-playerinv', (playerId) => {
    let pelurinv = getPlayerinv(playerId);
    io.emit('send-playerinv', pelurinv);
  } )
